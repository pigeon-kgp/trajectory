...
def auth():
	...
	dbs=raw_input("Enter database name (recommended: trajectory): ")
	usr=raw_input("Enter user name (recommended: ubuntu): ")
	print("Enter password (recommended: lion): ")
	passw=getpass.getpass()
	db_con = psycopg2.connect(database=dbs, user=usr, password=passw, host="127.0.0.1", port="5432")
	cursor = db_con.cursor()
	cursor.execute("ROLLBACK")
	
def write(taxino,lat,lon,starttime,endtime,count,precision):
	global cursor
